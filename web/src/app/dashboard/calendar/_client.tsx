...
const labelStyle: React.CSSProperties = {
  ...
  color: "var(--color-gold-dim)", marginBottom: "0.4rem",
};
...
function DayModal({ dateStr, workEntries, costEntries, users, userColors, onClose }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto p-6 pt-12"
      ...
      <div style={{ background: "#0d0a1a", border: "1px solid rgba(201,168,76,0.3)", borderRadius: "20px", padding: "2rem 2.25rem", width: "100%", maxWidth: 520, boxShadow: "0 24px 80px rgba(0,0,0,0.7)", animation: "fadeInUp 0.3s ease" }}>

        {/* Header */}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "1.25rem" }}>
          <div>
            <div style={{ fontFamily: "var(--font-cinzel)", fontSize: "0.58rem", letterSpacing: "0.2em", color: "var(--color-gold-dim)", marginBottom: "0.3rem", textTransform: "uppercase" }}>Napi bejegyzések</div>
            <div style={{ fontFamily: "var(--font-playfair)", fontSize: "1.15rem", color: "var(--color-gold-light)", textTransform: "capitalize" }}>{displayDate}</div>
          </div>
        ...
        {(dayRevenue > 0 || dayCosts > 0) && (
          <div style={{ display: "flex", gap: "0.6rem", marginBottom: "1.25rem", padding: "0.85rem 1rem", background: "rgba(255,255,255,0.03)", borderRadius: "12px", border: "1px solid rgba(201,168,76,0.12)" }}>
        ...
        {/* Divider */}
        <div style={{ borderTop: "1px solid rgba(201,168,76,0.12)", margin: "1rem 0" }} />

        {/* Tab switcher */}
        <div style={{ display: "flex", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(201,168,76,0.15)", borderRadius: "10px", padding: "3px", gap: "3px", marginBottom: "1.1rem" }}>
          {([["work", "◈ Munkadíj"], ["cost", "✦ Kiadás"]] as const).map(([key, label]) => (
            <button key={key} type="button" onClick={() => setTab(key)}
              style={{ flex: 1, padding: "0.5rem", border: "none", borderRadius: "7px", cursor: "pointer", fontFamily: "var(--font-cinzel)", fontSize: "0.62rem", letterSpacing: "0.12em", background: tab === key ? "rgba(201,168,76,0.18)" : "transparent", color: tab === key ? "var(--color-gold)" : "rgba(245,230,211,0.4)", transition: "all 0.2s" }}>
              {label}
            </button>
          ))}
        </div>
                ...
                {users.map(u => {
                  ...
                  return (
                    <button key={u.id} type="button" onClick={() => setUserId(u.id)}
                      style={{ padding: "0.45rem 0.9rem", borderRadius: "8px", cursor: "pointer", border: userId === u.id ? `1px solid ${col}88` : "1px solid rgba(255,255,255,0.08)", background: userId === u.id ? `${col}18` : "transparent", color: userId === u.id ? col : "rgba(245,230,211,0.45)", fontFamily: "var(--font-cormorant)", fontSize: "1rem", transition: "all 0.2s" }}>
                      {u.name}
                    </button>
                  );
                })}
            ...
            <div style={{ display: "flex", gap: "0.5rem" }}>
              {(Object.entries(COST_CONFIG) as [CostType, typeof COST_CONFIG[CostType]][]).map(([key, cfg]) => (
                <button key={key} type="button" onClick={() => setCostType(key)}
                  style={{ flex: 1, padding: "0.55rem", borderRadius: "8px", cursor: "pointer", border: costType === key ? `1px solid ${cfg.color}88` : "1px solid rgba(255,255,255,0.08)", background: costType === key ? `${cfg.color}18` : "transparent", color: costType === key ? cfg.color : "rgba(245,230,211,0.45)", fontFamily: "var(--font-cinzel)", fontSize: "0.6rem", letterSpacing: "0.12em", transition: "all 0.2s" }}>
                  {cfg.icon} {cfg.label}
                </button>
              ))}
            </div>
...
const delBtnStyle: React.CSSProperties = { background: "none", border: "none", color: "rgba(245,230,211,0.2)", cursor: "pointer", fontSize: "0.9rem", padding: "0.2rem 0.35rem", borderRadius: "5px", transition: "color 0.2s" };

function SaveBtn({ loading }: { loading: boolean }) {
  return (
    <button type="submit" disabled={loading}
      style={{ padding: "0.8rem", border: "none", borderRadius: "10px", cursor: loading ? "not-allowed" : "pointer", fontFamily: "var(--font-cinzel)", fontSize: "0.68rem", fontWeight: 600, letterSpacing: "0.18em", color: "#07040f", background: "linear-gradient(120deg, #7a6229 0%, #c9a84c 50%, #7a6229 100%)", backgroundSize: "200% auto", animation: "shimmer 3s linear infinite", boxShadow: "0 4px 16px rgba(201,168,76,0.25)", opacity: loading ? 0.7 : 1 }}>
...
function DayColumn({ date, workEntries, costEntries, userColors, isToday, onOpen, compact = false }: {
  ...
  return (
    <div style={{ flex: 1, minWidth: 0, background: isToday ? "rgba(201,168,76,0.04)" : "transparent", border: isToday ? "1px solid rgba(201,168,76,0.22)" : "1px solid rgba(201,168,76,0.08)", borderRadius: "14px", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      {/* Header */}
      <div onClick={() => onOpen(dateStr)}
        style={{ padding: compact ? "0.55rem 0.7rem" : "0.85rem 1rem", borderBottom: "1px solid rgba(201,168,76,0.1)", cursor: "pointer", background: isToday ? "rgba(201,168,76,0.1)" : "rgba(255,255,255,0.02)", transition: "background 0.2s" }}
        ...
        <div style={{ fontFamily: "var(--font-cinzel)", fontSize: "0.56rem", letterSpacing: "0.1em", color: isToday ? "var(--color-gold)" : "var(--color-gold-dim)", textTransform: "uppercase" }}>{DAYS_L[dow]}</div>
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between" }}>
          <div style={{ fontFamily: "var(--font-playfair)", fontSize: compact ? "1.05rem" : "1.35rem", color: isToday ? "var(--color-gold)" : "var(--color-cream)", lineHeight: 1.1 }}>
            {date.getDate()}
            {!compact && <span style={{ fontSize: "0.78rem", color: "rgba(245,230,211,0.35)", marginLeft: "0.35rem" }}>{MONTHS[date.getMonth()]}</span>}
          </div>
          {profit !== 0 && (
      ...
      <div style={{ padding: "0.55rem", flex: 1, display: "flex", flexDirection: "column" }}>
        ...
        <div onClick={() => onOpen(dateStr)}
          ...
          onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = "rgba(201,168,76,0.15)"; (e.currentTarget as HTMLElement).style.color = "rgba(201,168,76,0.28)"; }}>
          + bejegyzés
        </div>
      </div>
    </div>
  ...
  return (
    <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(201,168,76,0.15)", borderRadius: "20px", overflow: "hidden" }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", borderBottom: "1px solid rgba(201,168,76,0.1)" }}>
        {DAYS_S.map(d => <div key={d} style={{ padding: "0.6rem 0", textAlign: "center", fontFamily: "var(--font-cinzel)", fontSize: "0.56rem", letterSpacing: "0.15em", color: "var(--color-gold-dim)", textTransform: "uppercase" }}>{d}</div>)}
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)" }}>
        {cells.map((day, idx) => {
          if (!day) return <div key={`e${idx}`} style={{ minHeight: 88, borderRight: "1px solid rgba(201,168,76,0.06)", borderBottom: "1px solid rgba(201,168,76,0.06)", background: "rgba(0,0,0,0.12)" }} />;
          ...
          return (
            <div key={ds}
              style={{ minHeight: 88, padding: "0.38rem", borderRight: "1px solid rgba(201,168,76,0.06)", borderBottom: "1px solid rgba(201,168,76,0.06)", background: isToday ? "rgba(201,168,76,0.05)" : "transparent", cursor: "pointer", transition: "background 0.18s", position: "relative" }}
              ...
              {/* Day number */}
              <div style={{ width: 22, height: 22, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--font-cinzel)", fontSize: "0.62rem", color: isToday ? "#07040f" : "rgba(245,230,211,0.45)", background: isToday ? "var(--color-gold)" : "transparent", marginBottom: "0.25rem" }}>{day}</div>
              ...
              <div onClick={() => onOpen(ds)} style={{ padding: "0.12rem 0.35rem", borderRadius: "4px", border: "1px dashed rgba(201,168,76,0.13)", color: "rgba(201,168,76,0.22)", fontSize: "0.6rem", textAlign: "center", cursor: "pointer", fontFamily: "var(--font-cinzel)", transition: "all 0.18s" }}
                ...
                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = "rgba(201,168,76,0.22)"; (e.currentTarget as HTMLElement).style.borderColor = "rgba(201,168,76,0.13)"; }}>+</div>
  ...
  return (
    <div style={{ animation: "fadeInUp 0.5s ease" }}>
      ...
      <div style={{ marginBottom: "1.5rem" }}>
        <h1 style={{ fontFamily: "var(--font-playfair)", fontSize: "2rem", color: "var(--color-gold-light)", animation: "float 4s ease-in-out infinite" }}>Munkanaptár ✦</h1>
        <p style={{ fontStyle: "italic", color: "var(--color-rose)", opacity: 0.75, fontFamily: "var(--font-cormorant)" }}>Ki mikor dolgozott, mennyit keresett, és mi a napi profit</p>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "0.75rem", marginBottom: "1.25rem" }}>
        ...
        <div style={{ display: "flex", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(201,168,76,0.18)", borderRadius: "10px", padding: "3px", gap: "3px" }}>
          {VIEW_BTNS.map(({ key, label }) => (
            <button key={key} onClick={() => setView(key)}
              style={{ padding: "0.42rem 0.85rem", border: "none", borderRadius: "7px", cursor: "pointer", fontFamily: "var(--font-cinzel)", fontSize: "0.6rem", letterSpacing: "0.1em", background: view === key ? "rgba(201,168,76,0.2)" : "transparent", color: view === key ? "var(--color-gold)" : "rgba(245,230,211,0.45)", transition: "all 0.2s" }}>
              {label}
            </button>
          ))}
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}>
          ...
          <span style={{ fontFamily: "var(--font-cinzel)", fontSize: "0.78rem", letterSpacing: "0.1em", color: "var(--color-gold)", minWidth: 190, textAlign: "center", textTransform: "capitalize" }}>{navLabel()}</span>
          <button onClick={() => navigate(1)} style={navBtnStyle}>›</button>
        </div>
