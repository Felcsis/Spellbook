        ...
        <div style={{ display: "flex", background: "var(--bg-panel)", border: "1px solid var(--border)", borderRadius: "10px", padding: "3px", gap: "3px", marginBottom: "1.1rem" }}>
          {([["work", "◈ Munkadíj"], ["cost", "✦ Kiadás"]] as const).map(([key, label]) => (
            <button key={key} type="button" onClick={() => setTab(key)}
              style={{ flex: 1, padding: "0.5rem", border: "none", borderRadius: "7px", cursor: "pointer", fontFamily: "var(--font-cinzel)", fontSize: "0.62rem", letterSpacing: "0.12em", background: tab === key ? "var(--border)" : "transparent", color: tab === key ? "var(--color-teal)" : "var(--text-soft)", transition: "all 0.2s" }}>
              {label}
            </button>
          ))}
        </div>
        ...
        <div style={{ display: "flex", background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "10px", padding: "3px", gap: "3px" }}>
          {VIEW_BTNS.map(({ key, label }) => (
            <button key={key} onClick={() => setView(key)}
              style={{ padding: "0.42rem 0.85rem", border: "none", borderRadius: "7px", cursor: "pointer", fontFamily: "var(--font-cinzel)", fontSize: "0.6rem", letterSpacing: "0.1em", background: view === key ? "var(--border)" : "transparent", color: view === key ? "var(--color-teal)" : "var(--text-soft)", transition: "all 0.2s" }}>
              {label}
            </button>
          ))}
        </div>
