...
export default function NapiClient({ isAdmin = true, userId = "" }: { isAdmin?: boolean; userId?: string }) {
  ...
  const workerStats = allUsers
    .map(u => {
      const rev   = allDayEntries.filter(e => e.type === "revenue"  && e.createdBy?.id === u.id).reduce((s, e) => s + e.amount, 0);
      const wages = allDayEntries.filter(e => e.type === "wage"     && e.createdBy?.id === u.id).reduce((s, e) => s + e.amount, 0);
